...
const upstreamRemote = gitRemotes.find((remote) =>
  remote.includes('BorisTB/pokedex.git')
);
if (upstreamRemote) {
  ...
  console.error(
    'No upstream remote found for BorisTB/pokedex.git. Skipping comparison against upstream main.'
  );
}
...
if (exitCode === 0) {
  ...
  console.log(
    '\nEXAMPLE: \n' +
      'feat(pokedex): add an option to generate lazy-loadable modules\n' +
